...
replaceMissingValsWithMostCommon = False
firstTestIndex = 2000
testSize = 1500
            ...
            for line in f:
                ...
                for currIter, element in enumerate(lineElements):
                    ...
                    if currIter not in [1,6,13] or element != '?':
                        ...
                        if replaceMissingValsWithMostCommon == False:
                            if currIter == 1:
                                if is50k == 0:
                            ...
                            if currIter == 1:
                                insertArr.append('Private')
                            elif currIter == 6:
                                insertArr.append('Prof-specialty')
                            else:
                                insertArr.append('United-States')

                if firstTestIndex <= lineNum < firstTestIndex + testSize:
                    testWriter.writerow(insertArr)
                else:
                    trainWriter.writerow(insertArr)
                lineNum += 1
print()